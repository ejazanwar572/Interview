...
DROP TABLE IF EXISTS products_rd;
...
DROP TABLE IF EXISTS purchases_rd;
...
(SELECT a.* , b.category 
FROM purchases_rd a
LEFT JOIN products_rd b ON a.product_id = b.product_id
)
--
SELECT a.user_id , count(DISTINCT product_id) as uniq_prods
FROM base a
WHERE category = 'Electronics'
GROUP BY 1
HAVING uniq_prods = (SELECT COUNT(DISTINCT product_id) FROM base WHERE category = 'Electronics') 
ORDER BY 2 DESC
