...
--
SELECT COUNT(DISTINCT CONCAT(b.user_id,b.event_timestamp))*100/COUNT(DISTINCT CONCAT(a.user_id,a.event_timestamp))  as view_to_cart_rate
     , COUNT(DISTINCT c.user_id)*100/COUNT(DISTINCT b.user_id)  as cart_to_purchase_rate
FROM page_views a
LEFT JOIN cart_add b ON a.user_id = b.user_id AND TIMESTAMPDIFF(HOUR,b.event_timestamp,a.event_timestamp) <= 24
LEFT JOIN purchase c ON b.user_id = c.user_id AND TIMESTAMPDIFF(HOUR,c.event_timestamp,b.event_timestamp) <= 24
