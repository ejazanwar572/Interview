...
WHERE 
    ...
    (t4.id IS NOT NULL AND t2.id IS NOT NULL)
    OR
    -- Case 3: t5, t4, t1 are consecutive (t1 is end) -> t1-2, t1-1, t1
    (t5.id IS NOT NULL AND t4.id IS NOT NULL)
ORDER BY visit_date;
...
WHERE people >= 100 
  AND (
       (prev1 >= 100 AND prev2 >= 100) -- Current is 3rd
    OR (prev1 >= 100 AND next1 >= 100) -- Current is 2nd
    OR (next1 >= 100 AND next2 >= 100) -- Current is 1st
  )
ORDER BY visit_date;
