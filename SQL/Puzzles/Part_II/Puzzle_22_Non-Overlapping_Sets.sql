...
BEGIN
    ...
    WHILE @i <= LENGTH(@string1) AND @result = 0
    BEGIN
        SET @char = SUBSTRING(@string1, @i, 1);

        IF INSTR(@string2 COLLATE Latin1_General_BIN, @char) > 0 AND PATINDEX(@pattern, @char) > 0
        BEGIN
            SET @result = 1;
        END
...
(
SELECT  OrderID, MySet, MAX(LENGTH(MySet) - LENGTH(REPLACE(MySet,',','')) + 1) AS MaxNumberSets
...
BEGIN
    ...
    (
    SELECT  a.*, b.Value
    FROM    OrdersDetermineCombinations a CROSS JOIN LATERAL
    ...
    cte_StringAgg AS
    (
    SELECT  RowNumber, GROUP_CONCAT(ValueTranslation ORDER BY ValueTranslation ASC SEPARATOR ',') AS MySetOrdered
