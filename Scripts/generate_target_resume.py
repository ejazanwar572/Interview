...
def generate_resume(input_md_path, output_pdf_path):
    # CSS Template aiming to match 'Suchithra_PR _DA_FT.pdf'
    ...
    /* Custom Bullet (Dash) to match Suchithra format */
    li::before {
        content: "–"; 
        position: absolute;
        left: -1.2em; 
    ...
    try:
        ...
        print(f"SUCCESS: Target Format PDF generated at {output_pdf_path}")
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print("Usage: python generate_target_resume.py <input_md_file> <output_pdf_file>")
    else:
