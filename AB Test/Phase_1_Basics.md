...
| **Exponential** | Continuous | Time *between* events in a Poisson process. Memoryless property. | Predicting time until the next customer purchase or server failure. |
...
*   **Difference from CLT:** LLN tells you *where* the center is going (accuracy); CLT tells you about the *shape* of the distribution around that center (precision/normality).
...
*   **P-value:** The probability of observing a test statistic as extreme as, or more extreme than, the one observed, **assuming $H_0$ is true**.
    *   *Interview Tip:* Never say "Probability that $H_0$ is true." Say "Probability of data given $H_0$."
...
*The most common "gotcha" question.*
...
*   **Key Insight:** SE decreases as sample size ($n$) increases (because dividing by $\sqrt{n}$). SD does *not* decrease with $n$ (it stabilizes).
